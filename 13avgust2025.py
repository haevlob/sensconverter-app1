...
class SensitivityConverter(BoxLayout):
    ...
    def setup_conversion_data(self):
        ...
        # Старая таблица для сенсы между PUBG и CoD
        self.pubg_cod_manual_sens_old = {
        ...
        # Старая таблица для гироскопа между PUBG и CoD
        self.pubg_cod_manual_gyro_old = {
            "general_3p": {0: 0, 100: 31, 200: 64, 400: 126, 5456: 1719},
            "general_1p": {0: 0, 100: 31, 200: 64, 400: 126, 5456: 1719},
            "col": {0: 0, 100: 31, 200: 64, 400: 126, 5456: 1719},
            "2x": {0: 0, 124: 25, 246: 48, 490: 108, 6683: 1473},
            "3x": {0: 0, 72: 13, 150: 27, 326: 58, 4446: 791},
            "4x": {0: 0, 54: 10, 113: 21, 245: 45, 3342: 614},
            "6x": {0: 0, 36: 7, 76: 14, 163: 31, 2224: 423},
            "6x_sniper": {0: 0, 44: 8, 83: 15, 187: 34, 2550: 464},
            "8x": {0: 0, 30: 5, 61: 10, 123: 22, 1664: 300}
        }
    ...
    def create_widgets(self):
        top_frame = BoxLayout(orientation='horizontal', size_hint_y=None, height=120)
        self.add_widget(top_frame)

        left_game_frame = BoxLayout(orientation='vertical')
        top_frame.add_widget(left_game_frame)
        left_game_frame.add_widget(Label(text="Исходная игра", size_hint_y=None, height=30))
        ...
        right_game_frame.add_widget(Label(text="Целевая игра", size_hint_y=None, height=30))
        ...
        accel_frame = BoxLayout(orientation='vertical', size_hint_y=None, height=60)
        self.add_widget(accel_frame)
        accel_frame.add_widget(Label(text="Ускорение проведения по горизонтали", size_hint_y=None, height=30))
        self.accel_inner_frame = BoxLayout(orientation='horizontal')
        accel_frame.add_widget(self.accel_inner_frame)

        settings_frame = BoxLayout(orientation='vertical', size_hint_y=None, height=120)
        self.add_widget(settings_frame)

        mode_frame = BoxLayout(orientation='horizontal', size_hint_y=None, height=60)
        settings_frame.add_widget(mode_frame)
        mode_frame.add_widget(Label(text="Режим:", size_hint_x=None, width=60))
        self.mode_buttons = {}
        ...
        sensor_frame = BoxLayout(orientation='horizontal', size_hint_y=None, height=60)
        settings_frame.add_widget(sensor_frame)
        sensor_frame.add_widget(Label(text="Тип:", size_hint_x=None, width=60))
        self.sensor_buttons = {}
        ...
        self.table_frame = GridLayout(cols=4, spacing=5, size_hint_y=None, row_force_default=True, row_default_height=40)
    ...
    def setup_acceleration_ui(self):
        ...
        if 'pubg' in [left_game, right_game]:
            ...
            self.accel_inner_frame.add_widget(Label(text="PUBG", size_hint_x=None, width=100))
            ...
            if other_game == 'standoff':
                self.accel_inner_frame.add_widget(Label(text="Standoff", size_hint_x=None, width=100))
                ...
                self.accel_inner_frame.add_widget(Label(text="CoD", size_hint_x=None, width=100))
    ...
    def setup_conversion_table(self):
        ...
        header_label = Label(text=f"{left_header} - {right_header}", font_size=16, size_hint_y=None, height=40)
        ...
        for standoff_label, pubg_label, cod_label, key in rows:
            ...
            font_size = 10 if 'кол' in left_label_text.lower() else 12

            add_left = left_label_text or left_game == "standoff" or (left_game == "pubg" and key == "6x_sniper")

            if add_left:
                left_label = Label(text=left_label_text, font_size=font_size, size_hint_y=None, height=40, halign='right', valign='middle')
                ...
                left_input = TextInput(text='', multiline=False, readonly=state, input_filter='float', size_hint_y=None, height=40)
                self.table_frame.add_widget(left_input)
                self.left_widgets.append((left_input, key, left_label_text))
            else:
                self.table_frame.add_widget(Label(text='', size_hint_y=None, height=40))
                left_input = TextInput(text='', multiline=False, readonly=True, size_hint_y=None, height=40)
                self.table_frame.add_widget(left_input)

            right_label = Label(text=right_label_text, font_size=font_size, size_hint_y=None, height=40, halign='right', valign='middle')
            right_label.bind(size=right_label.setter('text_size'))
            self.table_frame.add_widget(right_label)

            right_input = TextInput(text='', multiline=False, readonly=True, input_filter='float', size_hint_y=None, height=40)
    ...
    def update_manual_conversion(self, key, right_input, left_input, left_label, value, *args):
        ...
        if left_game == "standoff" and right_game == "pubg":
            ...
            source_data = self.pubg_cod_manual_sens_old if sensor == "sensitivity" else self.pubg_cod_manual_gyro_old
            ...
            if key == "6x_sniper":
                for _, ri, k, _, _ in self.entry_widgets:
                    if k == "6x":
                        calculated_value = self.invert_interpolate(left_value, source_data, "6x")
                        ri.text = str(int(round(calculated_value)))
