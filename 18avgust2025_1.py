...
class SensitivityConverter(BoxLayout):
    ...
    def setup_acceleration_ui(self):
        ...
        if 'pubg' in [left_game, right_game]:
            ...
            if other_game == 'standoff':
                ...
                self.standoff_accel_input = TextInput(text=str(self.standoff_accel), multiline=False, readonly=pubg_side == 'left', input_filter='float', height=dp(40), size_hint_y=None, padding=[dp(5), dp(5), dp(5), dp(5)], font_size=dp(14))
                if pubg_side == 'left':
                ...
                self.cod_accel_input = TextInput(text=str(self.cod_accel), multiline=False, readonly=pubg_side == 'left', input_filter='int', height=dp(40), size_hint_y=None, padding=[dp(5), dp(5), dp(5), dp(5)], font_size=dp(14))
                if pubg_side == 'left':
            ...
            self.accel_inner_frame.add_widget(Label(text="Standoff", size_hint_x=None, width=dp(100)))
            self.standoff_accel_input = TextInput(text=str(self.standoff_accel), multiline=False, input_filter='float', height=dp(40), size_hint_y=None, padding=[dp(5), dp(5), dp(5), dp(5)], font_size=dp(14))
            self.accel_inner_frame.add_widget(self.standoff_accel_input)
            self.standoff_accel_input.bind(text=self.on_standoff_accel_text)

            self.accel_inner_frame.add_widget(Label(text="CoD", size_hint_x=None, width=dp(100)))
            self.cod_accel_input = TextInput(text=str(self.cod_accel), multiline=False, readonly=True, input_filter='int', height=dp(40), size_hint_y=None, padding=[dp(5), dp(5), dp(5), dp(5)], font_size=dp(14), background_color=[0.8, 0.8, 0.8, 1], foreground_color=[0, 0, 0, 1])
            self.accel_inner_frame.add_widget(self.cod_accel_input)

        elif left_game == 'cod' and right_game == 'standoff':
            self.accel_inner_frame.add_widget(Label(text="CoD", size_hint_x=None, width=dp(100)))
            self.cod_accel_input = TextInput(text=str(self.cod_accel), multiline=False, input_filter='int', height=dp(40), size_hint_y=None, padding=[dp(5), dp(5), dp(5), dp(5)], font_size=dp(14))
            self.accel_inner_frame.add_widget(self.cod_accel_input)
            self.cod_accel_input.bind(text=self.on_cod_accel_text)

            self.accel_inner_frame.add_widget(Label(text="Standoff", size_hint_x=None, width=dp(100)))
            self.standoff_accel_input = TextInput(text=str(self.standoff_accel), multiline=False, readonly=True, input_filter='float', height=dp(40), size_hint_y=None, padding=[dp(5), dp(5), dp(5), dp(5)], font_size=dp(14), background_color=[0.8, 0.8, 0.8, 1], foreground_color=[0, 0, 0, 1])
            self.accel_inner_frame.add_widget(self.standoff_accel_input)
    ...
    def update_accel_inputs(self):
        if hasattr(self, 'standoff_accel_input'):
            self.standoff_accel_input.text = f"{self.standoff_accel:.2f}"
        if hasattr(self, 'cod_accel_input'):
            self.cod_accel_input.text = str(self.cod_accel)
    ...
    def update_manual_conversion(self, key, right_input, left_input, left_label, value, *args):
        ...
        if left_game == "standoff" and right_game == "pubg":
            ...
            right_input.text = str(int(round(calculated_value)))
            if key == "6x":
                pubg_value = calculated_value
                sniper_cod = self.interpolate_value(pubg_value, source_data, "6x_sniper")
                for l_input, k, _ in self.left_widgets:
                    if k == "6x_sniper":
                        l_input.text = str(int(round(sniper_cod)))
                        break
            if key == "6x_sniper":
                for _, ri, k, _, _ in self.entry_widgets:
                    if k == "6x":
